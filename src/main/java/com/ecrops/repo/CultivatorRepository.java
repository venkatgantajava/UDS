...
public interface CultivatorRepository extends JpaRepository<Cultivator, Integer> {
	...
	@Query(value = "SELECT sum(occupant_extent) FROM cr_booking WHERE part_key = :part_key and kh_no = :kh_no and cr_vcode= :cr_vcode and cultivator_type IS NOT NULL and cr_sno = :cr_sno", nativeQuery = true)
	...
	@Query(value = "SELECT anubhavadar_extent FROM cr_booking WHERE part_key = :part_key and kh_no = :kh_no and cr_vcode= :cr_vcode  and owner_tenant = 'O' and cr_sno = :cr_sno limit 1", nativeQuery = true)
