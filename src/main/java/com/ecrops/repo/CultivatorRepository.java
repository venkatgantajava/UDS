...
public interface CultivatorRepository extends JpaRepository<Cultivator, Integer> {
	...
	@Query(value = "SELECT sum(occupant_extent) FROM ecrop2023.cr_booking WHERE part_key = :part_key and kh_no = :kh_no and cr_vcode= :cr_vcode", nativeQuery = true)
	...
	@Query(value = "SELECT anubhavadar_extent FROM ecrop2023.cr_booking WHERE part_key = :part_key and kh_no = :kh_no and cr_vcode= :cr_vcode  and owner_tenant = 'O'", nativeQuery = true)
